...
import { Box, Typography, Divider, Grid, Paper, Chip, Stack, Alert, Tabs, Tab } from '@mui/material'; // @mui/material v5.0.0
import { styled } from '@mui/material/styles'; // @mui/material/styles v5.0.0
import { ArrowBack, Edit, Delete, PlayArrow, Send, Cancel, Science, Assignment } from '@mui/icons-material'; // @mui/icons-material v5.0.0
...
/**
 * Page component that displays detailed information about an experiment
 */
const ExperimentDetailPage: React.FC<ExperimentDetailPageProps> = ({ experimentId }) => {
  // LD1: Get URL parameters using useParams hook to extract experiment ID
  ...
  // LD1: Set up effect to fetch experiment data when component mounts or ID changes
  useEffect(() => {
  ...
  // LD1: Create handleBack function to navigate back to experiments list
  const handleBack = useCallback(() => {
    navigate('/app/experiments');
  }, [navigate]);

  // LD1: Create handleStatusChange function to refresh experiment data
  ...
  }, [currentExperiment, editData, updateExperiment, showToast, handleStatusChange]);
  ...
  }, [currentExperiment, queueExperiment, showToast, handleStatusChange]);
  ...
  }, [currentExperiment, submitExperiment, showToast, handleStatusChange]);
  ...
  }, [currentExperiment, cancelExperiment, showToast, handleStatusChange]);
  ...
  const handleMoleculeSelectionChange = useCallback(
    ...
    [currentExperiment, addMoleculesToExperiment, removeMoleculesFromExperiment, showToast, handleStatusChange]
  );
  ...
  // LD1: Render the ExperimentDetail component with the experiment ID and handlers
  return (
    <DetailContainer>
      <DetailHeader>
      ...
      {activeTab === '2' && (
        <TabPanel>
          {currentExperiment.submissions && currentExperiment.submissions.length > 0 ? (
            <SubmissionDetail submissionId={currentExperiment.submissions[0].id} onStatusChange={handleStatusChange} />
          ) : (
